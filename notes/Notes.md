...
**The step formula.** 
From 'scr_steps' in the game's compiled code:
...
**Room transitions reset the counter.** 
Progress toward the next encounter is lost
...
An encounter object's 'Create' event sets the 
...
**Kill counters are per-flag, not per-area.** 
Each encounter object passes a flag index as its fourth argument: 202 Ruins, 203 Snowdin, 204 Waterfall, 205 Hotland *and* CORE. 
Hotland and the CORE therefore share one pool of 40 kills. There is no 206.

**Suppressed encounters in the early Ruins.** 
Encounters begin spawning in room 3, but kills are not counted until room 8. That time cannot be recovered by routing.
...
- **LV on arrival in an area is a range, not a fixed number.** 
  The kill count per area is fixed but the encounter pool is not, so total EXP depends on which
  ...
  However, it is generally not recommended to grind for Gyftrot and Ice Cap,
  as the prior is a rare encounter and would waste a lot of time walking around waiting for it to appear,
  and the latter takes multiple turns for ACTs like "Steal" to take effect.
